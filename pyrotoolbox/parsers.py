...
def _parse_workbench_calibration(analyte: str, line1: str, line2: str) -> dict:
    ...
    if analyte == 'oxygen':
        ...
        rename_dict = {'lastCal1': 'date_calibration_offset',
                       'M': 'M',
                       'N': 'N',
                       'C': 'C',
                       'Tofs (K)': 'temp_offset',
...
def _parse_developertool_settings(line1: str, line2: str) -> dict:
    ...
    for name, value in zip(line1.split('\t')[1:], line2.split('\t')[1:]):
        ...
        elif name in ('options (1=autoFlashDur., 2=autoAmp, 4= 1000xOxygen)',
...
def read_aquaphoxlogger(fname: str) -> tuple[pd.DataFrame, dict]:
    ...
    if metadata['firmware'] >= '405':
        ...
        metadata['sensor_code'] = (int(sensor_type).to_bytes(4, 'big').decode('ascii').split('\x00')[0] +
    ...
    df = pd.read_csv(fname, skiprows=header, encoding='latin1', usecols=usecols, sep='\t',
                     skip_blank_lines=False, na_values=['-300000'])
