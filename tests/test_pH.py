...
class Test_calc_top_and_bottom(unittest.TestCase):
    def test_workbench(self):
        self.assertEqual((1.78604717943155, 0.0950633788014184), pHmodule._calc_top_and_bottom(m_wb["calibration"]))

    def test_developertool(self):
        self.assertEqual((2.0405290903363387, 0.25619353351220603), pHmodule._calc_top_and_bottom(m_dt["calibration"]))

    def test_aquaphox(self):
        self.assertEqual((1.636112161867461, 0.04760775604660251), pHmodule._calc_top_and_bottom(m_ap["calibration"]))
    ...
    def test_developertool(self):
        self.assertEqual(
            round(
                calculate_pH(
                    df_dt["R"],
                    m_dt["settings"]["temperature"],
                    m_dt["settings"]["salinity"],
    ...
    def test_aquaphox(self):
        self.assertEqual(
            round(
                calculate_pH(
                    df_ap["R"],
                    df_ap["sample_temperature"],
                    m_ap["settings"]["salinity"],
...
class Test_calculate_pH_from_calibration(unittest.TestCase):
        ...
        self.assertEqual(
            round(
                calculate_pH_from_calibration(
                    df_wb["R"], df_wb["sample_temperature"], m_wb["settings"]["salinity"], m_wb["calibration"]
                ).iloc[2],
                3,
            ),
            df_wb["pH"].iloc[2],
    ...
    def test_developertool(self):
        self.assertEqual(
            round(
                calculate_pH_from_calibration(
                    df_dt["R"], m_dt["settings"]["temperature"], m_dt["settings"]["salinity"], m_dt["calibration"]
                ),
                3,
            ).iloc[0],
            df_dt["pH"].iloc[0],
    ...
    def test_aquaphox(self):
        self.assertEqual(
            round(
                calculate_pH_from_calibration(
                    df_ap["R"], df_ap["sample_temperature"], m_ap["settings"]["salinity"], m_ap["calibration"]
                ),
                3,
            ).iloc[1],
            df_ap["pH"].iloc[1],
...
class Test_calculate_pH_from_interpolated_calibrations(unittest.TestCase):
    ...
    def test_input_with_dict(self):
        pH = calculate_pH_from_interpolated_calibration(
            self.data["R"],
            self.data["sample_temperature"],
            self.metadata["settings"]["salinity"],
            calibrations={"2023-08-17 15:29:26": self.calibs1, "2023-11-15 13:05:26": self.calibs2},
        )
...
class Test_calculate_pH_with_prospective_drift_compensation(unittest.TestCase):
    ...
    def test_prospective(self):
        pH = calculate_pH_with_prospective_drift_compensation(
            self.data["R"],
            self.data["sample_temperature"],
            self.metadata["settings"]["salinity"],
            self.calibs1,
