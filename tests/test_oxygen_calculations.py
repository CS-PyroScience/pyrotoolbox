...
class Test_convert_to_hPa(unittest.TestCase):
    ...
    def test4(self):
        hpa = convert_to_hPa(
            self.data["oxygen_%airsat"], "oxygen_%airsat", pressure=self.pressure, temperature=self.temperature
        )
        for i in range(len(self.data)):
            self.assertAlmostEqual(self.data["oxygen_hPa"][i], hpa.iloc[i], 3)
    ...
    def test8(self):
        ...
        with self.assertRaises(ValueError):
            convert_to_hPa(
                self.data["oxygen_mg/L"], "oxygen_mg/L", pressure=self.pressure, temperature=self.temperature
            )
        with self.assertRaises(ValueError):
